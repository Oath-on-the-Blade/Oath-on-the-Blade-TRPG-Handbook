...
拆檔後 **25 KB 以上 Markdown：1 份；20 KB 以上：5 份**。目前最大的 10 份正文為：

- `審查/AI_GM手冊結構自動掃描.md` — 31053 bytes / 648 行
- `劇本設計規則/設計/場景遭遇與獎勵.md` — 24757 bytes / 482 行
- `劇本設計規則/設計/基礎規格與能力.md` — 21211 bytes / 251 行
- `劇本設計規則/可重覆任務設計.md` — 21057 bytes / 276 行
- `GM規則/開局檢查.md` — 20357 bytes / 204 行
- `GM規則/角色卡格式.md` — 18434 bytes / 297 行
- `GM規則/遊戲流程/判定與調查.md` — 17153 bytes / 210 行
...
自動精確段落比對（跨檔、至少 120 字）目前仍有 **1 群**。退役可重覆小型副本的整段規則重覆已移除。若剩餘重覆只是公式在「玩家規則」與「劇本設計」各自保留一段介面摘要，可視為刻意局部重述；其他長段應優先改成單一權威正文 + 路由／連結。
